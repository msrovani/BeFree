...
Abre uma proposta no módulo de governança com autor padrão sendo o DID do orquestrador. Define eventos `governance:proposal:created` e `governance:proposal:activated` quando apropriado.
...
Manipulam o status da proposta e emitem eventos específicos (`governance:proposal:cancelled`, `governance:proposal:closed`).
...
O orquestrador também emite eventos `storage:restored`, `storage:saved`, `storage:error` e uma família `governance:proposal:<evento>` para criação, ativação, voto, cancelamento e encerramento de propostas quando um adaptador está presente.
