...
export class CommunityOrchestrator extends EventEmitter {
  ...
  async createProposal(draft: ProposalDraft, options: { activate?: boolean } = {}) {
    ...
    if (options.activate && proposal.status !== 'active') {
      ...
      this.dispatchAutomation({ type: 'governance:proposal:activated', proposal: finalProposal });
  ...
  async cancelProposal(proposalId: string) {
    ...
    this.dispatchAutomation({ type: 'governance:proposal:cancelled', proposal });
    this.triggerPersist();
    return proposal;
  }
  ...
  async closeProposal(proposalId: string) {
    ...
    this.dispatchAutomation({ type: 'governance:proposal:closed', proposal });
    this.triggerPersist();
    return proposal;
  }
  ...
  async voteOnProposal(proposalId: string, vote: ProposalVoteOptions) {
    ...
    this.dispatchAutomation({ type: 'governance:proposal:voted', vote: record });
    this.triggerPersist();
    return record;
  }
