...
const sanitizeSentences = (value) =>
  value
    .split(/[.!?]+\s*/)
    .map((entry) => entry.trim())
    .filter(Boolean);

const buildJarbasSystemPrompt = ({
  memory,
  conversationSummary,
  userIntent,
  userEmotion,
...
const registerJarbasResponse = (memory, response, interactionSummary) => {
...
const evolveJarbasMemory = (memory, updates = {}) => ({
...
const validateJarbasResponse = (response, memory) => {
